...
Halloween <- function(year){
  ...
  if(nchar(year) == 2){
    if(as.numeric(year) <= 68){
      cat('Halloween in', paste0('20',year), 'falls on a', paste0(20,year, 10, 31) %>% ymd %>% lubridate::wday(label = T, abbr = FALSE) %>% as.character)
    }
    else{
      cat('Halloween in', paste0('19',year), 'falls on a', paste0(19,year, 10, 31) %>% ymd %>% lubridate::wday(label = T, abbr = FALSE) %>% as.character)
    }
  }
  else{
    cat('Halloween in', year, 'falls on a', paste0(year, 10, 31) %>% ymd %>% lubridate::wday(label = T, abbr = FALSE) %>% as.character)
  }
}